...
func main() {
	...
	// Download files and launch plugins in goroutine
	go func() {
		var err int
		appDir, _ := os.Getwd()
		...
		if isDebug == false {
			...
			for {
				_, err := os.Stat(filepath.Join(appDir, "resources"))
				if err == nil {
					break
				} else {
...
func pageInstall(w fyne.Window) *fyne.Container {
	...
	plugin.Global.Entry("PageInstall")
...
func pageEnd(path string) *fyne.Container {
	...
	plugin.Global.Entry("PageAfterInstall")
	...
	if err != nil {
		...
		plugin.Global.Entry("PageThanks")
